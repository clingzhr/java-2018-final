...
public class GUIrefresh  extends Thread{
    ...
    private ArrayList<SAVE_action> save_actions; //保存的动作
    ...
    public  GUIrefresh(Canvas mycanvans,Confront confront,Writer write,ArrayList<SAVE_action> save_actions,boolean is_reloaded,FileReader read) {
        this.mycanvas = mycanvans;
        this.confront = confront;
        this.is_finished = false; //判断
        this.ground = confront.ret_ground();
        this.res_image = null;
        this.winner = new Image(this.getClass().getClassLoader().getResource(new String("pic/胜利.png")).toString(),
                800, 400, false, false);
    ...
    public void run() {
        //播放BGM
        Media media = new Media(this.getClass().getClassLoader().getResource(new String("music/bgm1.mp3")).toString());
        MediaPlayer mp = new MediaPlayer(media);
        mp.setAutoPlay(true);
        mp.setCycleCount(10); //十次
        mp.play();
    ...
    public void check()
    {
        ...
        synchronized (this.ground){ //上锁，其他进程不能访问，通过其获得状态
            is_Calash_Alive = this.ground.isCalashAlive(); //葫芦娃是否活着
             is_Monster_Alive = this.ground.isMonsterAlive(); //怪物是否活着
        }
    ...
    private void reviewRefresh_by_one_flash(ArrayList<String> arrayList) {
        Battlefield ground =this.confront.ret_ground();
    ...
    private void removeFromBattle(Creature cre) {
        try {
            synchronized (this.ground) {
                this.ground.remove(cre.getI(),cre.getJ());
            }
        }
        catch (Exception e) {
            e.printStackTrace();
